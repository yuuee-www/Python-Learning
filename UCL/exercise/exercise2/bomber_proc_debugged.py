...
''' update_position takes a list of x and y coordinates and pair of offsets.
    It creates a new list of x and y coordinates by adding the offsets to all
...
''' is_inside_building tests if point pos is inside building number
    ...
    for building_num in range(0, 1200//SPACING - 3):
        if (is_inside_building(building_num, plane_nose_pos, building_width,
            ...
            or is_inside_building(building_num, plane_wing_pos, building_width,
                                  building_heights, building_xpos)) :
            game_over(canvas)
    if plane_body_pos[1] > CANVAS_HEIGHT-50 and plane_body_pos[0] < 124:
        plane_landed(canvas)
